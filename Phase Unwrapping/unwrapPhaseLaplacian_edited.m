function unwrappedPhase = unwrapPhaseLaplacian2(wrappedPhase)
...
% wrappedPhase=load_nii_data('NN_HIE_001_3149523_R_Ax_QSM_20170612101156_14_phase.nii.gz');
    N = size(wrappedPhase);
    ...
    del_phase=ifftn(fftn(cos(wrappedPhase).* ifftn( fftn(sin(wrappedPhase)).*del_op)))-ifftn(fftn(sin(wrappedPhase) .* ifftn( fftn(cos(wrappedPhase)).*del_op)));
    
    % for i=1:20
    unwrappedPhase = wrappedPhase+ 2*pi*round((del_phase-wrappedPhase)/(2*pi));
 
   % wrappedPhase=unwrappedPhase;
    
    %end
 
     niftiwrite(unwrappedPhase,'phase_unwrap2');
end
